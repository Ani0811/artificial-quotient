    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5">
        {brandsList.map((brand, idx) => (
          ...
          >
            {/* Card Header: Brand Summary on left, Action Buttons on right */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-brand-border/60 dark:border-[#16382e]/80">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-white/5 border border-brand-border dark:border-[#16382e] p-1 flex items-center justify-center shrink-0">
                  {brand.logoUrl ? (
                    <img src={brand.logoUrl} alt={brand.name} className="w-full h-full object-contain" />
                  ) : (
                    <span className="text-xs font-bold text-emerald-500">{brand.logoText || brand.name?.charAt(0) || "B"}</span>
                  )}
                </div>
